...
export async function downloadFromS3(file_key: string): Promise<string> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Get the system's temp directory (works on Windows, Linux, macOS)
      const tempDir = os.tmpdir();

      // Generate the file name with the timestamp
      const name = `mahdi${Date.now().toString()}.pdf`;

      // Combine the temp directory and file name to create the full file path
      const file_name = path.join(tempDir, name);
